...
In NICo, we call "Leaf" the device that the host (which we make available for tenants) plugs into.
This is typically a DPU that will make the overlay network available
to the tenant. In future iterations, the Leaf might be a specialized switch instead of a DPU.
...
NICo software images as well as the images that the tenant
wants to run.
